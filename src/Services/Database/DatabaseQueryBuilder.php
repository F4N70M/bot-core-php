...
use PDOException;
use PDOStatement;
use TgBotCore\Services\Database\DatabaseException;
	...
	* SHOW *
	...
	public function show($type, $from = null)
	{
		...
		if (!(in_array($showType,['TABLES','VARIABLES']) OR (in_array($showType,['COLUMNS','INDEX']) && $from !== null)))
			throw new TgBotCore\Exception("Invalid argument \"$type\" must be TABLES|COLUMNS|INDEX|VARIABLES");
...
//                $value = $this->intersectTableData($parts['into'],$value);
	...
	protected function prepare($sql, array $bind)
	{
		...
		{
			throw new DatabaseException($e->errorInfo[2]);
			// echo "Request failed: ";
			// echo $e->getMessage();
			// $statement = null;
		}
